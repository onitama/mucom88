...
public:
	...
	SubWindowBase(HWND hw,HWND hOwnWnd){
		...
		SetWindowLongPtr(hw,GWLP_WNDPROC,(LONG_PTR)Hook);	//	設定なり自分自身をフック
		SetWindowLongPtr(hw,GWLP_USERDATA,(LONG_PTR)this);				//	自分登録
	};
	//	----------デスクトラクター----------
	virtual ~SubWindowBase(){
		SetWindowLongPtr(m_hWnd,GWLP_WNDPROC,(LONG_PTR)proc);	//	ポインタ復元
	}
	//	----------コールバックフック用----------
