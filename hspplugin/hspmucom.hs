...
p1,p2
p1(0) : バンクNo.
p2(0) : 再生フラグ(0=再生、1=準備のみ)

%inst
指定されたバンクNo.の音楽を再生します。
最初に、mucomload命令によりMUCOM88音楽データ(.mub)を読み込んでおく必要があります。
エラーが発生した場合は、システム変数statに0以外が代入されます。
p2パラメーターに0以外を設定した場合は、再生の準備だけを行った上で再生はスキップします。
音色の状態やトラックの情報のみを取得する場合に設定してください。
...
var,"filename",option
var        : 取得されたデータが代入される変数
"filename" : .mubまたは.mucファイルのパス
option(0)  : 0の時は.mucファイル、1の時は.mubファイルを読み込む

%inst
#driverタグで指定されたドライバー名を取得します。
option値で.mucファイル、または.mubファイルの種別を指定して、"filename"でパス名を指定します。
varで指定した変数に結果が代入されます。値は、MUCOM_DRIVER_*で示される値になります。
#driverタグが設定されていない場合は、MUCOM_DRIVER_NONE(未定義)となります。
また、不明なドライバー名の場合は、MUCOM_DRIVER_UNKNOWN(不明)が返されます。
...
%prm
p1
p1(0) : ドライバー名を示す値(MUCOM_DRIVER_*)

%inst
p1で指定したドライバー値をシステムに設定します。値は、MUCOM_DRIVER_*で示される値になります。
^p
   ドライバー名  : マクロ名                : 内容
 ------------------------------------------------------------------------
   "mucom88"       MUCOM_DRIVER_MUCOM88      オリジナルのドライバ(1.7)
   "mucom88e"      MUCOM_DRIVER_MUCOM88E     オリジナルのドライバ(1.5)
   "mucom88em"     MUCOM_DRIVER_MUCOM88EM    拡張メモリ版ドライバ(1.7)
   "mucomdotnet"   MUCOM_DRIVER_MUCOMDOTNET  MucomDotNET(外部ドライバ)
^p
以降は、ドライバー値で設定された内容で動作します。
デフォルトでは、MUCOM_DRIVER_MUCOM88が設定されており、mucomresetで設定がリセットされることはありません。
...
%inst
MML内で使用されているすべてのFM音色データを、音色定義文字列として取得します。
あらかじめ、mucomload命令で.mubファイルを読み込み、mucomplay命令で再生準備を行っておく必要があります。
結果は、varで指定された変数に文字列型として代入されます。
